      ...
      <div className="grid grid-cols-1 md:grid-cols-[clamp(200px,18%,280px)_minmax(0,1fr)] w-full min-h-screen">
        ...
        <main className="min-w-0 px-[clamp(18px,4vw,44px)] pt-6 pb-[60px] w-full max-w-[760px]">
          <div className="animate-pulse">
            <div className="h-3.5 w-36 rounded-full bg-warm-3 mb-7" />
            <div className="h-7 w-60 rounded-lg bg-warm-3 mb-3" />
            <div className="h-3.5 w-44 rounded-full bg-warm-2 mb-7" />
            <div className="grid gap-3.5">
              {Array.from({ length: rows }).map((_, i) => (
                <div
                  key={i}
                  className="h-[84px] rounded-[14px] border border-warm-3 bg-white flex items-center gap-4 px-5"
                >
