...
function Frame({ children }: { children: React.ReactNode }) {
  return (
    <svg viewBox="0 0 24 24" className="w-[26px] h-[26px]" aria-hidden="true" focusable="false">
      <g {...STROKE}>{children}</g>
    </svg>
  );
}
...
export default function ModuleIcon({ href }: { href: string }) {
  return <>{ICONS[href] ?? Vocabulary}</>;
}
