...
export default function ChapterQuiz({ questions }: { questions: QuizQuestion[] }) {
    ...
    return (
      <div className="border border-dashed border-[#D6D3D1] rounded-[14px] px-5 py-4 flex items-center gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          ...
          <span className="text-[12.5px] text-[#A1A1AA]">Totally optional — completing the day never depends on it.</span>
        </div>
        <button
          onClick={() => {
            reset();
  ...
  if (finished) {
    ...
    return (
      <div className="border border-[#E7E5E4] rounded-[14px] p-5 text-center">
        <p className="text-[24px] mb-1">{perfect ? "🌟" : "👏"}</p>
        <p className="text-[15px] font-extrabold mb-1">
          {correct}/{questions.length} correct
        </p>
        <p className="text-[12.5px] text-[#71717A] mb-3.5">
          {perfect ? "Perfect!" : "Re-read the sections above for the ones you missed."}
        </p>
        <div className="flex gap-2 justify-center">
          <button
            onClick={reset}
            className="rounded-[9px] px-[18px] py-2 text-[13px] font-semibold text-[#18181B] bg-white border border-[#E7E5E4] hover:bg-[#FAFAF9] transition-colors"
          >
            Try again
          </button>
          <button
  ...
  return (
    <div className="border border-[#E7E5E4] rounded-[14px] p-5">
      <div className="flex items-center justify-between mb-1.5">
        <p className="text-[11px] font-bold tracking-[.07em] uppercase text-[#A1A1AA]">
          Practice quiz · {index + 1}/{questions.length}
        </p>
        <button
          onClick={() => setOpen(false)}
          className="text-[12px] text-[#A1A1AA] hover:text-[#18181B] transition-colors"
        >
          Stop ✕
        </button>
      </div>
      <p className="font-bold text-[15.5px] mb-0.5">{qq.q}</p>
      {qq.hint && <p className="text-[12.5px] text-[#A1A1AA] mb-1.5">{qq.hint}</p>}
