...
const COMPOUND_VOWELS: Record<string, StrokeAnchors[]> = {
  ...
  // ㅝ ㅞ: the ㅓ/ㅔ tick sits BELOW ㅜ's bar, as in print (워, 웨) — putting
  // the two on one line ran them together into a single long stroke.
  ㅝ: [[C(58, 192), C(170, 192)], [C(114, 192), C(114, 266)], [C(160, 226), C(214, 226)], [C(214, 40), C(214, 280)]],
  ㅞ: [[C(58, 192), C(162, 192)], [C(110, 192), C(110, 266)], [C(150, 226), C(202, 226)], [C(202, 52), C(202, 268)], [C(238, 52), C(238, 268)]],
  ㅟ: [[C(78, 204), C(200, 204)], [C(138, 204), C(138, 266)], [C(234, 52), C(234, 268)]],
  ㅢ: [[C(64, 226), C(202, 226)], [C(224, 52), C(224, 268)]],
};
...
const W_VOWEL_BAR_TOP: Record<string, number> = { ㅘ: 174, ㅙ: 178, ㅚ: 178, ㅝ: 192, ㅞ: 192, ㅟ: 204, ㅢ: 226 };
...
const CHO_DROP: Record<string, [number, number]> = {
  ㅗ: [26, 58], ㅛ: [10, 42], ㅠ: [12, 40], ㅡ: [12, 40], ㅜ: [12, 26],
};
...
function syllableAnchors(char: string): StrokeAnchors[] {
  ...
  if (VERTICAL_VOWELS.has(jung)) {
    ...
    const choBox: Box = [cx - 32, cx + 32, 70, barTop - 30];
