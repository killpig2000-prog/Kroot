...
import GRAMMAR_EXAMPLE_OVERRIDES from "@/lib/grammar-example-overrides.json";
...
// Extra examples generated by scripts/gen-grammar-examples.mts, appended
// after each section's hand-authored ones — most sections only had 2-3,
// thin for internalizing a pattern. Keyed by "{lessonKey}:{sectionIndex}".
const EXTRA_EXAMPLES = GRAMMAR_EXAMPLE_OVERRIDES as Record<string, GrammarExample[]>;

function withExtraExamples(lesson: GrammarLesson): GrammarLesson {
  return {
    ...lesson,
    sections: lesson.sections.map((section, i) => {
      const extra = EXTRA_EXAMPLES[`${lesson.key}:${i}`];
      return extra?.length ? { ...section, examples: [...section.examples, ...extra] } : section;
    }),
  };
}
