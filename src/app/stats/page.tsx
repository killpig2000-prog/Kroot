...
export const metadata = {
  title: "Insights — Kroot",
  description: "Your detailed learning stats: accuracy by skill, weakest words, and XP timeline.",
};
...
export default async function StatsPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("display_name, streak_days, avatar_url, plus_until")
    .eq("id", user.id)
    .single();
      ...
      <div className="grid grid-cols-1 md:grid-cols-[clamp(200px,18%,280px)_minmax(0,1fr)] w-full min-h-screen">
        ...
        <main className="min-w-0 px-[clamp(18px,4vw,44px)] pt-6 pb-[100px] md:pb-[60px]">
          <div className="flex gap-2 text-[13px] text-[#A19A8C] mb-[18px]">
            ...
            <span>/</span>
            <b className="text-[#18181B] font-semibold">Insights</b>
          </div>
          ...
          <div className="flex items-center justify-between gap-4 mb-[18px] flex-wrap">
            <h1 className="font-bold text-[22px] tracking-[-0.02em] flex items-center">
              ...
              Insights
              <span className="ml-2.5 text-[10.5px] font-bold tracking-[.04em] text-[#92400E] bg-[#FFFBEB] border border-[#FDE68A] rounded-md px-1.5 py-0.5">
                🌟 PLUS
              </span>
            </h1>
          </div>
  ...
  // ---- locked teaser for free plans ----
  if (!plusActive) {
    return shell(
      <div className="max-w-[820px]">
        <div className="border border-[#FDE68A] bg-[#FFFBEB] rounded-[14px] px-[26px] py-8 text-center">
          <span className="text-[34px] block mb-2">📊</span>
          <b className="block font-bold text-[18px] tracking-[-0.01em] mb-1.5">
            See exactly how your Korean is growing
          </b>
          <p className="text-[13.5px] text-[#6B6560] max-w-[440px] mx-auto leading-[1.65] mb-5">
            Accuracy by skill, your weakest words ready for extra watering, how deep each word is
            rooted, and a day-by-day XP timeline — all in one place, for Plus members.
          </p>
          <Link
            href="/pricing"
            className="inline-flex rounded-[10px] bg-[#16A34A] px-5 py-2.5 text-[13.5px] font-bold text-white hover:bg-[#15803D] transition-colors"
          >
            Unlock with Kroot Plus →
          </Link>
        </div>

        {/* what's inside, blurred */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3.5 select-none" aria-hidden="true">
          {[
  ...
  const [{ data: xpRows }, { data: vocabRows }, { data: readingRows }, { data: speakingRows }] =
    await Promise.all([
      ...
      supabase
        ...
        .eq("user_id", user.id),
      supabase.from("speaking_progress").select("best_score").eq("user_id", user.id),
    ]);
