    ...
    <div className="min-h-screen bg-[#FDFBF7] text-[#18181B]">
      ...
      <main className="max-w-[880px] mx-auto px-[clamp(18px,4vw,28px)] py-[clamp(40px,6vw,72px)]">
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-[7px] rounded-full border border-[#BBF7D0] bg-[#F0FDF4] px-3.5 py-1.5 text-[12.5px] font-semibold text-[#16A34A] mb-4">
        ...
        {plusActive && (
          <div className="border border-[#FDE68A] bg-[#FFFBEB] rounded-[14px] px-5 py-4 mb-8 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          {/* free plan */}
          <div className="border border-[#E3DDD0] rounded-2xl p-6 flex flex-col">
            <b className="font-semibold text-[15px] mb-1">Free</b>
            <p className="text-[26px] font-extrabold tracking-[-0.02em] mb-4">
              $0 <span className="text-[13px] font-medium text-[#A19A8C]">forever</span>
            </p>
            <ul className="flex flex-col gap-2 text-[13px] text-[#6B6560] mb-6">
              {FREE_FEATURES.map((f) => (
            ...
            <div className="mt-auto">
              <Link
                href="/onboarding"
                className="w-full inline-flex items-center justify-center rounded-[10px] border border-[#E3DDD0] bg-white px-[22px] py-[11px] text-[14px] font-semibold hover:bg-[#FAF7EF] transition-colors"
              >
                Start free
              </Link>
            </div>
            ...
            return (
              <div
                ...
                className={`rounded-2xl p-6 flex flex-col border ${
                  highlight ? "border-[#16A34A] shadow-[0_12px_32px_rgba(22,163,74,.12)]" : "border-[#E3DDD0]"
                }`}
              >
                ...
                <p className="text-[26px] font-extrabold tracking-[-0.02em] mb-4">
                  {plan.price} <span className="text-[13px] font-medium text-[#A19A8C]">{plan.per}</span>
                </p>
                <ul className="flex flex-col gap-2 text-[13px] text-[#6B6560] mb-6">
                  <li className="flex gap-2">
                    <span className="text-[#16A34A]">✓</span> Everything in Free
                  </li>
                  {PLUS_PERKS.map((p) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-12">
          {PLUS_PERKS.map((p) => (
            <div key={p.title} className="border border-[#E3DDD0] rounded-[14px] px-5 py-4">
              <b className="block font-semibold text-[14px] mb-1">
                {p.icon} {p.title}
              </b>
              <p className="text-[13px] text-[#6B6560] leading-[1.55]">{p.desc}</p>
            </div>
          ))}
        </div>
