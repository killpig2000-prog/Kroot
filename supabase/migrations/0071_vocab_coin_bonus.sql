-- Vocabulary's per-Day coin bumped 15 -> 20, same 60% pass/fail gate as
-- every other skill (no separate "perfect" tier — that would make vocab the
-- only skill with a 3-way payout and give the UI a new thing to explain).
-- The 5-Day daily cap from 0070 is unchanged, so it now tops out at
-- 5 x 20 = 100 coins/day instead of 75.
--
-- Everything else is identical to 0070; only c_activity_bonus_coins's use
...
begin
  ...
  v_bonus_coins := case when p_skill = 'vocabulary' then c_vocab_bonus_coins else c_activity_bonus_coins end;
