-- /review's coin payout was flat (the same 20 coins from 0071 whether the
-- learner answered the base 10 words or bought capacity up to 40) — buying
-- capacity (migration 0056, coins) got no bigger a reward for the bigger
-- session. Now 2 coins per word actually answered (matching the already-
-- established "10 words = 20 coins" rate from vocab Days), passed in as the
-- new p_review_count — untouched for every other caller, which leaves it
-- null and falls back to the flat per-skill bonus as before.
--
-- Also excludes the review item from the 5-Day/day vocab cap (0070): review
-- is a different mechanic with its own natural ceiling (dailyReviewCap, 10-40
-- words), and sharing one budget with vocab Day completions was an
-- unintended side effect of keying both under skill = 'vocabulary'.

drop function if exists public.award_xp(integer, text, text, smallint, public.cefr_level);
...
create function public.award_xp(
  ...
  -- Words actually answered this call (full session or an early exit's
  -- partial count). Only meaningful for the "review" item; every other
  -- caller omits it and gets the flat per-skill bonus.
...
declare
  ...
  c_vocab_bonus_coins constant integer := 20;
...
begin
  ...
  v_bonus_coins := case
    ...
    when p_skill = 'vocabulary' then c_vocab_bonus_coins
