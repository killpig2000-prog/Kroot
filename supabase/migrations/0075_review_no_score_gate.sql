-- A review session no longer has to clear the 60% accuracy gate to pay.
--
-- Review is the one activity that is *supposed* to surface words the learner
-- got wrong — a word marked "still learning" comes back the next day by
-- design. Gating its coins on scoring 60% therefore punished exactly the
-- learner who rated themselves honestly: their sessions are full of the words
-- they admitted not knowing, they score low, and they earn nothing. The
-- learner who taps "Got it" on everything gets an easy review and the coins,
-- while poisoning their own SRS schedule. Paying per word answered (0072,
-- 2/word) already scales the reward to effort, so the gate added nothing here.
--
-- The gate stays exactly as it was everywhere else, including the vocabulary
-- Day payout — clicking through ten new words without reading them should
-- still earn nothing.
--
-- Everything else is byte-identical to 0074.
...
begin
  ...
  -- A review session is exempt: getting words wrong is what review is for,
  -- and its reward already scales with how many words were answered. Set
  -- before every use so the coin gate, the top-up path and coins_blocked all
  -- agree that accuracy was never the blocker here.
  v_score_ok := v_is_review or p_score is null or p_score >= c_min_score;
  ...
  v_capped := p_skill = 'vocabulary' and not v_is_review and v_vocab_paid_today >= c_vocab_daily_coin_cap;
  ...
  -- Why nothing was paid, for the results screen. Only meaningful while the
  -- coins are still claimable at all (an item paid on an earlier attempt has
  -- nothing left to explain), and ordered by what the learner would have to
  -- change: level and the daily cap can't be fixed by trying harder, so they
  -- outrank the score hint that used to be the only message.
  if v_activity_coins = 0 and v_item_key is not null and (v_first or v_topup) then
    v_blocked := case
      when v_below_level then 'below_level'
      when v_capped then 'daily_cap'
      when not v_score_ok then 'score'
      else null
    end;
  end if;
