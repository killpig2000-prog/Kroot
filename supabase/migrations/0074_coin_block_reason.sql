-- The results screen could only say one thing when coins didn't land:
-- "score 60%+ for coins". Since 0070 there are three different reasons a
-- completion pays nothing, and two of them have nothing to do with accuracy —
-- a learner who scored 100% but hit the day's vocab cap, or who is redoing
-- content below their level, was told to score higher, which would never
-- work. The function already knows which case it is; it just wasn't saying.
--
-- Adds two read-only columns to the return:
--   coins_blocked   'score' | 'daily_cap' | 'below_level' | null
--                   why this call paid no activity coins, when it didn't.
--                   null when they were paid, or when nothing was claimable
--                   (no item key, already paid on an earlier attempt).
--   coins_possible  what this item pays when unblocked — 15 flat, or 2/word
--                   for a review session. The screen hardcoded 15, which
--                   would have gone stale the next time any rate moved.
--
-- Everything else is byte-identical to 0073.
